...
//            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
    ...
    private void calcOffset(int lastTab, int current) {
        final View currentTabView = mTabsContainer.getChildAt(current);
        mCurrentP.left = currentTabView.getLeft();
        mCurrentP.right = currentTabView.getRight();

        final View lastTabView = mTabsContainer.getChildAt(lastTab);
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    public void setCurrentTab(int currentTab) {
        ...
        //这里需要post一下，以便seTextSize之后对布局重新排列完成后再进行动画
        //否则还是拿的老布局的getLeft，指示器动画无法居中
        if (mIndicatorAnimEnable) {
            //因为是异步的，可能在post的时候mLastTab和mCurrentTab变化了，
            //为了保证数据一致，写了一个新的runnable
            post(new AniRunnable(mLastTab, mCurrentTab));
        } else {
            invalidate();
        }
    }
    ...
    public void setMsgMargin(int position, float leftPadding, float bottomPadding) {
        ...
        if (tipView != null) {
            TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
            final boolean select = getCurrentTab() == position;
            if (select) {
    ...
    private class AniRunnable implements Runnable {
        private int mLast;
        private int mCurrent;

        public AniRunnable(int last, int current) {
            this.mLast = last;
        ...
        public void run() {
            if (mLast >= 0 && mCurrent >= 0
                    && mLast < mTabEntitys.size() && mCurrent < mTabEntitys.size())
                calcOffset(mLast, mCurrent);
        }
    }
